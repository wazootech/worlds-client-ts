...
type MatchFunction = (
  ...
  rdfjs.Quad
>; /**  * RdfjsPatchBuffer owns insert/delete buffers and RDF/JS Store mutation helpers  * shared by durable hexastore-backed Store implementations.  */
export class RdfjsPatchBuffer {
  /** insertBuffer collects quads queued for insertion. */ private insertBuffer:
    rdfjs.Quad[] = []; /** deleteBuffer collects quads queued for deletion. */
  ...
  } /**    * flushBuffer deduplicates buffers and persists through commitHandler or fallback.    */
  public async flushBuffer(
    ...
    if (commitHandler) await commitHandler(patch, context);
    else if (fallbackCommitHandler) await fallbackCommitHandler(patch, context);
    this.clearBuffer();
